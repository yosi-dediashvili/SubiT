...
```python
def extract_identifiers(title, query): pass
```

Within it, the module will have several methods for extracting identifiers.

We'll have to different algorithm for the extraction. The first is for movies,
and the second, will be for title. They'll share some of the logic.

##### Extracting identifiers for movie titles

**1. Decide what will be the input for the algorithm and normalize it:**
...
After choosing the input, we'll apply the normalization method to the string,
and apply the following algorithm in order to generate a list from all the 
normalized names:
