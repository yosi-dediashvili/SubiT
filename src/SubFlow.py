...
class SubFlow(object):
    ...
    def _yield_movie_sub_stage_from_all_providers(self, query, full_path):
        """ Will yield MovieSubStages one at a time (like a generator). """
        reload_module(SubProviders)
        while setNextSubProvider():
            yield QuerySubStage(getSubProvider().PROVIDER_NAME, query,
    ...
    def _get_movie_sub_stage_from_all_providers(self, query, full_path):
        ...
        movie_sub_stages = []
        while setNextSubProvider():
            movie_sub_stages.extend(QuerySubStage(
    ...
    def _download_version_sub_stage\
        ...
        download_path is the full path for the subtitle file to be saved with.
        If a subtitle downloaded successfully, the function will return True,
        otherwise, False. 
        """
        try:
    ...
    def _handle_version_sub_stage(self, version_sub_stage):
        """ The function is responsible for handling the final result of the
        version_sub_stage. If the function return True, everything is OK, and 
        we should stop the SubFlow, otherwise, we need to continue.
        """
        succeeded = False
        if version_sub_stage:
            ...
            for i in range(2):
                ...
                succeeded = self._download_version_sub_stage\
                    (version_sub_stage, self._single_input.getDownloadPath())
                if succeeded:
    ...
    def _process(self, interactive):
        ...
        if self._handle_version_sub_stage(version_sub_stage):
