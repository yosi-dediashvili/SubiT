...
def performrequest( domain, url, data = '', type = HttpRequestTypes.GET, more_headers = '', retry = False, is_redirection = False ):
    ...
    try:
        ...
        #Handle redirections
        if not len(response) and not is_redirection: #if response is empty, and also to avoid endless requests, only if we are not requesting redirected page
            res_url = got_response.msg.dict['location']
            if url not in res_url:
                WriteDebug('Got Redirection')
                return performrequest(domain,  res_url.replace('http://', '').replace(domain, ''), is_redirection=True)

    except:
        WriteDebug('Failed Getting: %s->%s' % (domain, url))
...
def myfilter(filter_func, items, format_func):
    """Self implementation of the filter function, allowing to set different value than the one given in items"""
    filtered_items = []
    for item in items:
        if filter_func(item):
...
def restart():
    #restart with params if args is exsist
    if len(sys.argv) > 1:
