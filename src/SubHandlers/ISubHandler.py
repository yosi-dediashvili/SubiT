...
#===============================================================================
    ...
    def findrelevantsubversions( filename, subMovie ):
        ...
        try:
            ...
            subVersions.sort( lambda c_ver, n_ver: 
                                        1 if 
                                            len( filename - set( n_ver.VerSum.lower()
                                                                  .replace('-', ' ').replace('.', ' ')
                                                                  .split(' ') ) ) 
                                            <
                                            len( filename - set( c_ver.VerSum.lower()
                                                                  .replace('-', ' ').replace('.', ' ')
                                                                  .split(' ') ) ) 
                                        else -1
                        )
    
            firstitem = set( subVersions[0].VerSum.lower().replace('-', ' ').replace('.', ' ').split(' ') )
    ...
    def findrelevantmovies( subSearch ):
            ...
            movieslist.sort( lambda c_ver, n_ver: 
                                        1 if
                                            ...
                                            len(set( map( str.strip, 
                                                          n_ver.MovieName.lower().replace('/', ' ').replace('-', ' ').replace('.', ' ').split(' ')
                                                        ) ) - moviename ) 
                                            <
                                            len(set( map( str.strip, 
                                                          c_ver.MovieName.lower().replace('/', ' ').replace('-', ' ').replace('.', ' ').split(' ')
                                                        ) ) - moviename )) 
                                            or
        ...
        return (movieslist[0], first_is_certain)